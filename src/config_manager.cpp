...
void DeserializeStruct(const toml::table& tbl, T& obj, const char* sectionName) {
  ...
  cpp26::reflect::forEachField<T>([&](const cpp26::reflect::FieldInfo& field) {
    if (field.type == cpp26::reflect::FieldType::Int) {
      if (const auto* val = section->get(field.name); val && val->is_integer()) {
        field.setInt(&obj, static_cast<int>(val->as_integer()->get()));
      }
    } else if (field.type == cpp26::reflect::FieldType::Float) {
      if (const auto* val = section->get(field.name); val && val->is_floating_point()) {
      ...
      if (const auto* val = section->get(field.name); val && val->is_boolean()) {
...
std::filesystem::path ConfigManager::GetConfigPath() { // NOLINT(readability-convert-member-functions-to-static)
  std::array<wchar_t, MAX_PATH> path{};
  if (GetModuleFileNameW(nullptr, path.data(), MAX_PATH) > 0) {
    std::filesystem::path pth(path.data());
    return pth.parent_path() / "config.toml";
  }
  return "config.toml";
}
...
void ConfigManager::CheckHotReload() {
  ...
  try {
    ...
    // Ignore exceptions during hot reload check
  }
}
...
void ConfigManager::ImportLegacyIni(const std::filesystem::path &iniPath) {
  ...
  m_mutableConfig.dlss.mode = static_cast<int>(
      GetPrivateProfileIntA("Settings", "DLSSMode", 5, path.c_str()));
  m_mutableConfig.fg.multiplier = static_cast<int>(
