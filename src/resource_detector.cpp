...
bool ResourceDetector::InitCommandList(ID3D12Device* pDevice) {
  if (m_cmdList) return true;

  HRESULT hr = pDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&m_cmdAlloc));
  if (FAILED(hr)) return false;

  hr = pDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_cmdAlloc.Get(), nullptr,
...
void ResourceDetector::UpdateHeuristics(ID3D12CommandQueue* pQueue) {
  ...
  if (m_fence && m_fence->GetCompletedValue() < m_fenceVal - 1 && m_fenceVal > 1) {
    // Still busy
    return;
  }

  // Read back previous results if they just finished
  if (m_fence && m_fence->GetCompletedValue() >= m_fenceVal - 1 && m_fenceVal > 1 && m_lastAnalyzedCandidate) {
    ScanResult result = {};
    if (HeuristicScanner::Get().GetReadbackResult(result)) {
      ...
      LOG_INFO("[Scanner] Async Result for {:p} | Var:{:.4f} Uniform:{} Data:{}",
               static_cast<void*>(m_lastAnalyzedCandidate.Get()), data.variance, result.isUniform ? "YES" : "NO",
               result.hasData ? "YES" : "NO");

      // Apply scoring (find the candidate in the list)
      auto it = std::find_if(m_motionCandidates.begin(), m_motionCandidates.end(),
                             [this](const ResourceCandidate& c) { return c.pResource == m_lastAnalyzedCandidate; });
  ...
  if (m_frameCount % 120 != 0) return;

  Microsoft::WRL::ComPtr<ID3D12Device> pDevice;
  pQueue->GetDevice(IID_PPV_ARGS(&pDevice));
  if (!pDevice) return;

  if (!HeuristicScanner::Get().Initialize(pDevice.Get())) return;

  if (!InitCommandList(pDevice.Get())) return;
  ...
  for (auto& cand : m_motionCandidates) {
    if (cand.score > 0.4f && cand.score < 2.0f) {
      if (m_heuristics.find(cand.pResource.Get()) == m_heuristics.end() ||
          (m_frameCount - m_heuristics[cand.pResource.Get()].lastCheckFrame > 600)) {
  ...
  if (HeuristicScanner::Get().AnalyzeTexture(m_cmdList.Get(), bestCandidate->pResource.Get(), dummy)) {
    m_cmdList->Close();
...
void ResourceDetector::NewFrame() {
  ...
  auto isStale = [currentFrame, this](const ResourceCandidate& cand) {
    ...
    if (cand.pResource == m_bestMotion || cand.pResource == m_bestDepth || cand.pResource == m_bestColor) {
  ...
  if (m_frameCount % resource_config::kCleanupInterval == 0) {
    ...
    // game
    if (m_colorCandidates.size() > 50 || m_depthCandidates.size() > 50 || m_motionCandidates.size() > 50) {
      LOG_INFO("Resource detector cache trimming (Frame {})", m_frameCount.load(std::memory_order_relaxed));
...
void ResourceDetector::RegisterDepthFromClear(ID3D12Resource* pResource, float clearDepth) {
  if (!pResource) return;
  std::unique_lock<std::shared_mutex> lock(m_mutex);
...
void ResourceDetector::RegisterMotionVectorFromView(ID3D12Resource* pResource, DXGI_FORMAT viewFormat) {
  ...
  if (mvScore <= 0.0f) {
    ...
    if (viewFormat == DXGI_FORMAT_R16G16_FLOAT || viewFormat == DXGI_FORMAT_R16G16_UNORM ||
        viewFormat == DXGI_FORMAT_R16G16_SNORM || viewFormat == DXGI_FORMAT_R16G16_SINT ||
        viewFormat == DXGI_FORMAT_R16G16_UINT || viewFormat == DXGI_FORMAT_R16G16_TYPELESS ||
        viewFormat == DXGI_FORMAT_R32G32_FLOAT || viewFormat == DXGI_FORMAT_R32G32_SINT ||
...
void ResourceDetector::RegisterResource(ID3D12Resource* pResource, bool allowDuplicate) {
  ...
  if (mvScore < 0.5f && depthScore < 0.5f && colorScore < 0.5f) {
    ...
    if (s_rejectLog++ % 300 == 0) {
      LOG_DEBUG("[RES] Rejected resource {:p} {}x{} fmt:{} mv={:.2f} "
                "depth={:.2f} color={:.2f}",
                static_cast<void*>(pResource), desc.Width, desc.Height, static_cast<int>(desc.Format), mvScore,
                depthScore, colorScore);
    }
    return;
  }
  ...
  if (s_acceptLog++ % 120 == 0) {
    LOG_INFO("[RES] Candidate {:p} {}x{} fmt:{} mv={:.2f} depth={:.2f} color={:.2f}", static_cast<void*>(pResource),
             desc.Width, desc.Height, static_cast<int>(desc.Format), mvScore, depthScore, colorScore);
  }
  ...
  if (mvScore >= 0.5f) {
    ...
    for (auto& cand : m_motionCandidates) {
      if (cand.pResource.Get() == pResource) {
        ...
        cand.score = mvScore;
        cand.seenCount = std::min<uint32_t>(cand.seenCount + 1, resource_config::kFrequencyHitCap);
        target = &cand;
        found = true;
        break;
      }
    ...
    if (!found) {
      ...
      if (!quietScan) {
        LOG_DEBUG("Found MV Candidate: {}x{} Fmt:{} Score:{:.2f}", desc.Width, desc.Height,
                  static_cast<int>(desc.Format), mvScore);
      }
    }
    float adjusted = mvScore;
    if (currentFrame - lastSeenFrame <= resource_config::kRecencyFrames) adjusted += resource_config::kRecencyBonus;
    if (target) {
      adjusted +=
          resource_config::kFrequencyBonus * (std::min<uint32_t>(target->seenCount, resource_config::kFrequencyHitCap) /
  ...
  if (depthScore >= 0.5f) {
    ...
    if (!found) {
      ...
      if (!quietScan) {
        LOG_DEBUG("Found Depth Candidate: {}x{} Fmt:{} Score:{:.2f}", desc.Width, desc.Height,
                  static_cast<int>(desc.Format), depthScore);
      }
    }
  ...
  if (colorScore >= 0.5f) {
    ...
    if (!found) {
      ...
      if (!quietScan) {
        LOG_INFO("[DLSSG] Found Color Candidate: {}x{} Fmt:{} Score:{:.2f}", desc.Width, desc.Height,
                 static_cast<int>(desc.Format), colorScore);
      }
    }
...
float ResourceDetector::ScoreMotionVector(const D3D12_RESOURCE_DESC& desc) {
  ...
  switch (desc.Format) {
    ...
    default: return 0.0f; // Not a likely MV format
  }
  ...
  if (desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET) score += 0.15f;

  if (desc.SampleDesc.Count > 1) score -= resource_config::kMsaaPenalty;
  if (desc.MipLevels > 1) score -= resource_config::kMipPenalty;
...
float ResourceDetector::ScoreDepth(const D3D12_RESOURCE_DESC& desc) {
  ...
  if (desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) score += 0.3f;
  // Deny-SRV flag often accompanies depth-only resources
  if (desc.Flags & D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE) score += 0.1f;
  if (desc.SampleDesc.Count > 1) score -= resource_config::kMsaaPenalty;
  if (desc.MipLevels > 1) score -= resource_config::kMipPenalty;
  if (m_expectedWidth > 0 && m_expectedHeight > 0) {
    float ratioW = static_cast<float>(desc.Width) / static_cast<float>(m_expectedWidth);
